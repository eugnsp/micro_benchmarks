...
void copy_tr_write_contig(const Matrix<double>& from, Matrix<double>& to)
{
	for (std::size_t col = 0; col < from.cols(); ++col)
		for (std::size_t row = 0; row < from.rows(); ++row)
...
void copy_tr_read_contig(const Matrix<double>& from, Matrix<double>& to)
{
	for (std::size_t row = 0; row < from.rows(); ++row)
		for (std::size_t col = 0; col < from.cols(); ++col)
			to(row, col) = from(col, row);
...
void copy_tr_mkl(const Matrix<double>& from, Matrix<double>& to)
{
