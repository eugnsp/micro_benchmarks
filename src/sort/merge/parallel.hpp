...
#else
	tbb::parallel_invoke([=]() { tbb_merge(first1, mid1, first2, mid2, dest); },
		[=]() { tbb_merge(mid1, last1, mid2, last2, dest_mid); });
#endif
}

template<class Rand_access_it>
void tbb_merge(Rand_access_it first, Rand_access_it mid, Rand_access_it last, Rand_access_it dest)
{
...
#else
		tbb::parallel_invoke([=]() { tbb_merge_sort_impl(first, mid, dest, !in_place); },
			[=]() { tbb_merge_sort_impl(mid, last, dest_mid, !in_place); });
#endif
